...
replaced by D, B would become E, and so on. The method is named after Julius Caesar, who used it to communicate
with his generals. ROT-13 ("rotate by 13 places") is a widely used example of a Caesar cipher where the shift is 13.
...
def encode_rot(string):
    return ''.join([key[i] if i.isalpha() else i for i in string])


def decode_rot(string):
    return ''.join([key[i] if i.isalpha() else i for i in string])

print(encode_rot('Caesar cipher? I much prefer Caesar salad!'))
print(decode_rot('Pnrfne pvcure? V zhpu cersre Pnrfne fnynq!'))